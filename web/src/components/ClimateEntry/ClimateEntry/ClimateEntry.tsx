import humanize from 'humanize-string'

import { Link, routes, navigate } from '@redwoodjs/router'
import { useMutation } from '@redwoodjs/web'
import { toast } from '@redwoodjs/web/toast'

import type { DeleteClimateEntryMutationVariables, FindClimateEntryById } from 'types/graphql'

const DELETE_CLIMATE_ENTRY_MUTATION = gql`
  mutation DeleteClimateEntryMutation($id: Int!) {
    deleteClimateEntry(id: $id) {
      id
    }
  }
`

const formatEnum = (values: string | string[] | null | undefined) => {
  if (values) {
    if (Array.isArray(values)) {
      const humanizedValues = values.map((value) => humanize(value))
      return humanizedValues.join(', ')
    } else {
      return humanize(values as string)
    }
  }
}

const jsonDisplay = (obj: unknown) => {
  return (
    <pre>
      <code>{JSON.stringify(obj, null, 2)}</code>
    </pre>
  )
}

const timeTag = (datetime?: string) => {
  return (
    datetime && (
      <time dateTime={datetime} title={datetime}>
        {new Date(datetime).toUTCString()}
      </time>
    )
  )
}

const checkboxInputTag = (checked: boolean) => {
  return <input type="checkbox" checked={checked} disabled />
}

interface Props {
  climateEntry: NonNullable<FindClimateEntryById['climateEntry']>
}

const ClimateEntry = ({ climateEntry }: Props) => {
  const [deleteClimateEntry] = useMutation(DELETE_CLIMATE_ENTRY_MUTATION, {
    onCompleted: () => {
      toast.success('ClimateEntry deleted')
      navigate(routes.climateEntries())
    },
    onError: (error) => {
      toast.error(error.message)
    },
  })

  const onDeleteClick = (id: DeleteClimateEntryMutationVariables['id']) => {
    if (confirm('Are you sure you want to delete climateEntry ' + id + '?')) {
      deleteClimateEntry({ variables: { id } })
    }
  }

  return (
    <>
      <div className="rw-segment">
        <header className="rw-segment-header">
          <h2 className="rw-heading rw-heading-secondary">
            ClimateEntry {climateEntry.id} Detail
          </h2>
        </header>
        <table className="rw-table">
          <tbody>
            <tr>
              <th>Id</th>
              <td>{climateEntry.id}</td>
            </tr><tr>
              <th>Station id</th>
              <td>{climateEntry.stationId}</td>
            </tr><tr>
              <th>Topic</th>
              <td>{climateEntry.topic}</td>
            </tr><tr>
              <th>Period</th>
              <td>{climateEntry.period}</td>
            </tr><tr>
              <th>Data set</th>
              <td>{climateEntry.dataSet}</td>
            </tr>
          </tbody>
        </table>
      </div>
      <nav className="rw-button-group">
        <Link
          to={routes.editClimateEntry({ id: climateEntry.id })}
          className="rw-button rw-button-blue"
        >
          Edit
        </Link>
        <button
          type="button"
          className="rw-button rw-button-red"
          onClick={() => onDeleteClick(climateEntry.id)}
        >
          Delete
        </button>
      </nav>
    </>
  )
}

export default ClimateEntry
