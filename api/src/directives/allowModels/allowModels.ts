...
const validate: ValidatorDirectiveFunc = ({ context, directiveArgs }) => {
  ...
  logger.debug(
    ...
    "currentUser in allowModels directive"
  );
  ...
  logger.debug(directiveArgs, "directiveArgs in allowModels directive");

  throw new Error("Implementation missing for allowModels");
};
